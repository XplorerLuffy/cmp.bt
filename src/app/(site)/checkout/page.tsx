...
export default function CheckoutPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      <h1 className="mb-6 text-2xl font-bold text-brand-night">Checkout</h1>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        {form.paymentMethod === "BANK_TRANSFER" && (
          <div className="rounded-lg border border-brand-moon bg-brand-moon/10 p-4 text-sm">
            <p className="mb-2 font-semibold text-brand-night">Bank Account Details</p>
        ...
        <div className="rounded-lg border border-brand-night/10 bg-white p-4">
          <div className="flex justify-between font-semibold text-brand-night">
            <span>Subtotal ({items.length} item{items.length !== 1 ? "s" : ""})</span>
            <span>{formatBTN(subtotal)}</span>
          </div>
        </div>
      ...
      <style>{`.input { width: 100%; border: 1px solid rgba(27,19,48,0.2); border-radius: 0.5rem; padding: 0.625rem 0.75rem; background: white; }`}</style>
    </div>
  );
}
...
function PaymentOption({
  ...
  return (
    ...
    >
      <p className="font-semibold text-brand-night">{label}</p>
